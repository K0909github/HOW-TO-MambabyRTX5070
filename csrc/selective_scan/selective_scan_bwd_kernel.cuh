...
// Forward declarations (launch helpers use these before their definitions)
template<
    int kNThreads_, int kNItems_,
    bool kIsEvenLen_, bool kIsVariableB_, bool kIsVariableC_,
    bool kDeltaSoftplus_, bool kHasZ_,
...
// bool -> std::true_type / std::false_type dispatch (MSVC-safe)
template <typename F>
__host__ __forceinline__ void ss_dispatch_bool(bool cond, F&& f) {
    if (cond) {
        f(std::true_type{});
    } else {
        f(std::false_type{});
...
// Keep preprocessor directives out of BOOL_SWITCH/lambda bodies.
template <typename KernelT>
__host__ __forceinline__ void selective_scan_maybe_set_max_dynamic_smem(KernelT kernel, int smem_bytes) {
#if !defined(USE_ROCM)
...
template<
    int kNThreads, int kNItems,
    bool kIsEvenLen, bool kIsVariableB, bool kIsVariableC,
    bool kDeltaSoftplus, bool kHasZ,
    typename input_t, typename weight_t>
__host__ __forceinline__ void selective_scan_bwd_launch_impl(SSMParamsBwd &params, cudaStream_t stream) {
    using Ktraits =
        Selective_Scan_bwd_kernel_traits<
            kNThreads, kNItems,
            kIsEvenLen, kIsVariableB, kIsVariableC, kDeltaSoftplus, kHasZ,
            input_t, weight_t>;

    constexpr int kSmemSize =
        Ktraits::kSmemSize + MAX_DSTATE * sizeof(typename Ktraits::scan_t)
        + (kNThreads + 4 * MAX_DSTATE) * sizeof(typename Ktraits::weight_t);

    dim3 grid(params.batch, params.dim);
    auto kernel = &selective_scan_bwd_kernel<Ktraits>;

    selective_scan_maybe_set_max_dynamic_smem(kernel, kSmemSize);
...
void selective_scan_bwd_launch(SSMParamsBwd &params, cudaStream_t stream) {
    ...
    const int key = (is_even_len ? 1 : 0)
        | ((params.is_variable_B ? 1 : 0) << 1)
        | ((params.is_variable_C ? 1 : 0) << 2)
        | ((params.delta_softplus ? 1 : 0) << 3)
        | ((has_z ? 1 : 0) << 4);

#define SS_BWD_CASE(EVEN, VARB, VARC, DSP, HASZ) \
    case ((EVEN) | ((VARB) << 1) | ((VARC) << 2) | ((DSP) << 3) | ((HASZ) << 4)): \
        selective_scan_bwd_launch_impl<kNThreads, kNItems, \
            (EVEN) != 0, (VARB) != 0, (VARC) != 0, (DSP) != 0, (HASZ) != 0, input_t, weight_t>(params, stream); \
        break

    switch (key) {
        SS_BWD_CASE(0, 0, 0, 0, 0);
        SS_BWD_CASE(1, 0, 0, 0, 0);
        SS_BWD_CASE(0, 1, 0, 0, 0);
        SS_BWD_CASE(1, 1, 0, 0, 0);
        SS_BWD_CASE(0, 0, 1, 0, 0);
        SS_BWD_CASE(1, 0, 1, 0, 0);
        SS_BWD_CASE(0, 1, 1, 0, 0);
        SS_BWD_CASE(1, 1, 1, 0, 0);

        SS_BWD_CASE(0, 0, 0, 1, 0);
        SS_BWD_CASE(1, 0, 0, 1, 0);
        SS_BWD_CASE(0, 1, 0, 1, 0);
        SS_BWD_CASE(1, 1, 0, 1, 0);
        SS_BWD_CASE(0, 0, 1, 1, 0);
        SS_BWD_CASE(1, 0, 1, 1, 0);
        SS_BWD_CASE(0, 1, 1, 1, 0);
        SS_BWD_CASE(1, 1, 1, 1, 0);

        SS_BWD_CASE(0, 0, 0, 0, 1);
        SS_BWD_CASE(1, 0, 0, 0, 1);
        SS_BWD_CASE(0, 1, 0, 0, 1);
        SS_BWD_CASE(1, 1, 0, 0, 1);
        SS_BWD_CASE(0, 0, 1, 0, 1);
        SS_BWD_CASE(1, 0, 1, 0, 1);
        SS_BWD_CASE(0, 1, 1, 0, 1);
        SS_BWD_CASE(1, 1, 1, 0, 1);

        SS_BWD_CASE(0, 0, 0, 1, 1);
        SS_BWD_CASE(1, 0, 0, 1, 1);
        SS_BWD_CASE(0, 1, 0, 1, 1);
        SS_BWD_CASE(1, 1, 0, 1, 1);
        SS_BWD_CASE(0, 0, 1, 1, 1);
        SS_BWD_CASE(1, 0, 1, 1, 1);
        SS_BWD_CASE(0, 1, 1, 1, 1);
        SS_BWD_CASE(1, 1, 1, 1, 1);
        default:
            // Unreachable
            break;
    }
...
void selective_scan_bwd_kernel(SSMParamsBwd params) {
    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            weight_t A_scaled;
            // NOTE: M_LOG2E may be undefined on some Windows toolchains; use a literal instead.
