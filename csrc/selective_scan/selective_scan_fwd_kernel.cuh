        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            for (int r = 0; r < kNRows; ++r) {
                ...
                if constexpr (!kIsComplex) {
                    ...
                    A_val[r] = complex_t(A_val[r].real_ * kLog2e, A_val[r].imag_);
                }
            }
...
template<
    int kNThreads, int kNItems, int kNRows,
    bool kIsEvenLen, bool kIsVariableB, bool kIsVariableC, bool kHasZ,
    typename input_t, typename weight_t>
__host__ __forceinline__ void selective_scan_fwd_launch_case(SSMParamsBase &params, cudaStream_t stream) {
    using Ktraits = Selective_Scan_fwd_kernel_traits<
        kNThreads, kNItems, kNRows,
        kIsEvenLen, kIsVariableB, kIsVariableC,
        kHasZ, input_t, weight_t>;
...
#define SS_FWD_CASE(EVEN, VARB, VARC, HASZ) \
    case ((EVEN) | ((VARB) << 1) | ((VARC) << 2) | ((HASZ) << 3)): \
        selective_scan_fwd_launch_case<kNThreads, kNItems, kNRows, \
            (EVEN) != 0, (VARB) != 0, (VARC) != 0, (HASZ) != 0, input_t, weight_t>(params, stream); \
        break
