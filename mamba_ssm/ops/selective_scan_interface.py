...
try:
    ...
    # Windows では triton の wheel が提供されないことが多い。
    # mamba_ssm は一部の補助関数で triton LayerNorm を参照するが、学習/推論に必須ではないため
    # torch 実装へフォールバックする。
    def _layer_norm_fwd(
        x,
        weight,
        ...
        **kwargs,
        ...
        if is_rms_norm:
            ...
            y = (x_f * rstd).to(weight.dtype) * weight
            if bias is not None:
                y = y + bias
            return (y.to(x.dtype), None, rstd)

        y = F.layer_norm(x.to(weight.dtype), x.shape[-1:], weight=weight, bias=bias, eps=eps).to(x.dtype)
        return (y, None, None)
